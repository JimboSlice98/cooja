...
public class Peer2PeerMoteBLS extends AbstractApplicationMote {
  ...
  public void receivedPacket(RadioPacket p) {
    ...
    if (messages.length > 0 && !messages[messages.length - 1].trim().isEmpty()) {
      try {
        fromNode = Integer.parseInt(messages[messages.length - 1].trim());
      } catch (NumberFormatException e) {
        // System.out.println("Error: Cannot parse integer from the last element: " + messages[messages.length - 1]);
        return;
      }
    } else {
      // System.out.println("Error: The last element is empty or missing.");
      return;
    }
